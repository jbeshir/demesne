...
| `sandbox_agent`    | Run an AI coding agent in a fresh sandbox against a caller-supplied prompt. Provider is inferred from `model` (defaults to the first enabled provider with configured credentials, Codex-first). Outbound HTTPS is restricted to the vendor proxy. Returns exit code, stdout, stderr, the `/out` host path, and the (indicative) cost summary. | [ref](docs/reference/tools/sandbox_agent.md) |
