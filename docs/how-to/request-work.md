...
When `model` is omitted, demesne uses the first enabled provider with configured credentials in Codex-first order: Codex `gpt-5.6-sol`, then Claude Code `sonnet`. Pass a model such as `gpt-5.6-terra`, `gpt-5.6-luna`, `gpt-5.5`, `gpt-5.4-mini`, `fable`, `opus`, `sonnet`, or `haiku` to select a provider and model explicitly. A model belonging to a disabled provider is unavailable.
