...
- **`own_usage_usd` and `total_usage_usd` are indicative.** They are derived from `usage.json` which is computed from demesne's embedded pricing table, not from actual billing. For Codex (ChatGPT-OAuth) and Claude Code (OAuth) users, real charges are against the respective subscription rather than per-request billing. See [Indicative cost reporting](../explanation/key-concepts.md#indicative-cost-reporting) and [`usage.json` reference](usage-json.md).
