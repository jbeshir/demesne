...
Demesne is an agent-agnostic, local, sandboxed agent-orchestration MCP server. You — the human running demesne — drive it through your agent of choice (Claude Code, Claude Desktop, VS Code, or any other MCP client). This doc tree is split by audience:
...
- [Edit the host MCP allowlist](how-to/edit-host-mcp-allowlist.md) — control which host MCP tools are re-exposed to sandboxed agents.
