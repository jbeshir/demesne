...
- **Image allowlist** — five accepted names for shell tools: `node` (`node:22`), `python` (`python:3.12`), `go` (`golang:1`, batteries-included with the Go toolchain + git + gcc + make), `anaconda` (`continuumio/anaconda3:latest`, the default), and `browser` (demesne-built locally on first use — Playwright JS + Chromium/Firefox/WebKit + Node, headless rendering works at `egress=none`; first build pulls a ~1.6 GB base). `sandbox_agent` and `sandbox_research` use the agent provider's own image, built locally from an embedded Dockerfile (today: `demesne-codex:<hash>` for the codex provider; `demesne-claude-code:<hash>` for the claude-code provider).
...
- **AllowedPaths** — env-configured allowlist (`DEMESNE_ALLOWED_PATHS`) of host paths under which inputs may be mounted or uploaded. Both the candidate path and the allowlist entries are symlink-resolved before the containment check, so symlink escapes are rejected.
