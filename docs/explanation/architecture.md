...
Child sandboxes spawned by `sandbox_agent` inherit the parent's read-only `/in` and share the parent's writable `/workspace`; their `/out` is `/out/child/<name>`. `sandbox_research` children are isolated — see [Nested sandboxes reference](../reference/nested-sandboxes.md) for the output-path convention, the copy-to-`/out` rule, and grandchild nesting. Each agent run writes `<out>/results.json` with `own_usage_usd` and `total_usage_usd` (summing the whole descendant tree).
