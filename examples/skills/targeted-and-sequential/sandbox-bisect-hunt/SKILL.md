...
status: alpha
description: Narrow down which commit, file, config flag, dependency version, or input subset introduced a defect, regression, performance cliff, or behaviour change — through a demesne-orchestrated binary search. An orchestrator captures a deterministic reproducer and a search axis, drives a sequential sandbox_script probe loop (one fresh sandbox per midpoint, log2(N) steps), confirms the culprit with a verify probe, then hands everything to a reader that writes a root-cause report with a suggested fix shape. Apply when the user knows something broke and wants to know exactly which change caused it — "bisect this regression", "find the commit that broke X", "which dependency version introduced this", "what changed to make this fail", "regression hunt". Report-only by default, routing into sandbox-feature-work when the user wants the fix applied. Skip for broad quality sweeps (use sandbox-quality-improvement), static defect catalogues (use sandbox-code-defect-survey), and new feature work (use sandbox-feature-work).
