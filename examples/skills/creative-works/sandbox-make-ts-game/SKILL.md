...
- **Launch every `sandbox_agent` child (design, implement, reviewers, fix) with `background: true` and poll with `sandbox_wait`.** Background dispatch gives explicit sequencing and progress control; wait for each child to finish before the next shared-`/workspace` phase begins, preventing races.
