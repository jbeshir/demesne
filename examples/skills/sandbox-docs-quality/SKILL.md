...
3. **Fan out four medium-tier `sandbox_agent` children** — one per lens — named `detect-<lens>` (lowercase DNS-1123: letters, digits, interior hyphens, ≤40 chars). Dispatch all four with `background: true` and poll with `sandbox_wait` so they run concurrently (blocking children, which the orchestrator issues one per turn, would run sequentially instead); four is the fixed count, not a tunable batch. Each child reads both docs and code from `/in/<repo>`, reports findings with **file:line for both the doc claim and the code ground-truth**, and writes its own `/out/REPORT.md`. "Clean on this axis" is a valid result — no manufactured findings.
