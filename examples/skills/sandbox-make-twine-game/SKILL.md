...
- **Pin a format version the image actually ships.** The `twine` image carries the formats bundled with Tweego — currently **Harlowe 3.1.0** and **SugarCube 2.30.0** (plus Harlowe 1/2, SugarCube 1, Snowman, Chapbook). The `:: StoryData` `format-version` must be one of these; pinning a newer version (e.g. Harlowe 3.3.x) fails the compile with `Story format ... is not available`. If unsure, run `tweego --list-formats` (or `ls $TWEEGO_PATH`) in the `twine` image and pin what's there.
