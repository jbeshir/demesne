...
4. For every background job, call `sandbox_wait(job_id)` with its long default and repeat only if it returns `running`. Accept it only when `status=succeeded`, `exit_code=0`, and `/out/child/<name>/REPORT.md` is nonempty. Retry a failed, cancelled, missing, or malformed report once under `<name>-retry`; then record that axis as a coverage gap. Do not claim coverage for it.
