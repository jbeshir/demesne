...
2. **Explore + render plan** (orchestrator, reading `/workspace/repo`). Reuse any existing harness/fixtures rather than inventing a server. Then pick the **render matrix** by exploration:
   ...
   - **Compound states** — when the FE has more than one floating/overlaid layer that can appear together (a modal, popover, tooltip, dropdown, drawer, sticky toolbar, or map/canvas overlay control), add at least one state where two such layers are simultaneously visible — prefer the narrowest viewport, where floating chrome is densest and stacking-order defects are most likely. A layer that renders underneath a sibling only when both are open is invisible to a matrix that always captures floating layers in isolation (see Lens 5 criterion 9 in `lenses.md`).
