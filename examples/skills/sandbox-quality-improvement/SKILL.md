...
3. **AUDIT** — spawn one medium-tier `sandbox_agent` per dimension in parallel (names: `audit-correctness`, `audit-security`, `audit-design`, `audit-completeness`, `audit-types`; lowercase DNS-1123 — letters, digits, interior hyphens, ≤40 chars; bad names produce invalid volume names and poison sibling spawns). Batch ≤4 concurrent — a recommended limit, not a demesne-enforced cap. Each reads the codebase and `git -C /workspace/repo log`, then writes `/out/AUDIT-<dim>.md`: findings each with file:line, a severity tier (blocking / non-blocking), and a one-line rationale. Must not re-flag gate-covered issues; findings without file:line evidence are rejected. Dimension scope:
