...
description: Drive a wide-coverage exploration through a demesne swarm — a slow-tier orchestrator writes an exploration spec and a per-child parameter set, dispatches 15–50 fully isolated children (sandbox_script for deterministic sweeps, sandbox_agent for generative brainstorming) in batches of ≤4 with zero inter-child coordination, then a slow-tier aggregator harvests all findings and writes four deliverables: consensus, outliers, ranked candidates, and a methodology report. Apply when the value is in diversity of starting points rather than a coordinated plan: parameter sweeps, random-seed variation, Monte Carlo simulation, fuzz input variation, hypothesis-space traversal, N-angle brainstorming, scenario exploration, design-alternative generation. Triggers include "explore the space", "try N random seeds", "brainstorm N angles", "parameter sweep", "fuzz this", "generate diverse alternatives", "what scenarios could lead to X", "sweep the hypothesis space", "surface minority views", "run a swarm". Skip for coordinated multi-dimension research with an explicit avenue plan (use sandbox-product-research), fixed-taxonomy defect detection (use sandbox-code-defect-survey), fix-and-land quality work (use sandbox-quality-improvement), feature builds (use sandbox-feature-work), and single open-web lookups (call sandbox_research directly). Report-only — the swarm surfaces; the host decides.
