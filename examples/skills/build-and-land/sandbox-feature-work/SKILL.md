...
3. Have `implement-1` change `/workspace/repo`, write `/out/SUMMARY.md`, and run no landing operation. For every background job, call `sandbox_wait` repeatedly (each call ≤120 seconds). Advance only when `status=succeeded`, `exit_code=0`, and the required nonempty artifact exists. On failure/cancellation/missing output, retry once with a new name; then stop, preserve stderr, and write `/out/FAILURE.md`.
