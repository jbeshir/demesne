...
status: alpha
description: Apply ONE specified edit operation — a codemod, rename, dependency-version bump, config-file rewrite, doc-frontmatter migration, license-header insertion, deprecated-API replacement — to N similar target files in parallel across a repo. An orchestrator enumerates targets, shards them (≤20 files each), fans out parallel editors each in its own git worktree, runs per-shard verification, quarantines failing shards, merges survivors onto an integration branch, and delivers /out/repo for the host to fetch. Apply when one well-specified edit must land on many similar files. Triggers include "apply this codemod across the repo", "bump X to version Y everywhere", "rename this API across all packages", "add license headers to all Go files", "replace all uses of deprecated Foo with Bar". Skip for a single substantial code change (use sandbox-feature-work), an open-ended quality audit (use sandbox-quality-improvement), and regenerating docs to match code (use uplift-docs).
