...
3. Dispatch at most four editors at a time. Each writes `/out/CHANGES.md` and `/out/SUMMARY.md`. Call `sandbox_wait` with its long default and repeat only if it returns `running`; accept only `succeeded`, `exit_code=0`, and nonempty artifacts. Retry once with a unique name; then cancel dependents, quarantine the shard, preserve stderr, and record it in `FAILURES.md`.
