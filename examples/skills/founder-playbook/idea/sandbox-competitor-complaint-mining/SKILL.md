...
4. Poll every job with `sandbox_wait(job_id, timeout_seconds: 120)` until it is no longer `running`. Accept a result only when `status == succeeded`, `exit_code == 0`, and both declared files exist. Retry a failed, cancelled, or missing-output job once with a new unique `-r2` name. On a second failure, append `{name, job_id, status, exit_code, reason}` to `/workspace/quarantine.jsonl`; do not treat it as evidence.
