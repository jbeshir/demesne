...
This document chains the `examples/skills/sandbox-*` skills that implement the
activities in Anthropic's *The Founder's Playbook: Building an AI-Native Startup*
across its four stages. Each stage lists the skills in invocation order, what flows
from one to the next, and the playbook's own exit criteria for moving on. Coverage
of every numbered playbook activity is tabulated in [`COVERAGE.md`](COVERAGE.md);
the skills themselves live in [`examples/skills/`](examples/skills/README.md).
...
1. **`sandbox-hypothesis-stress-test`** — turn the vague observation into a
...
2. **`sandbox-market-landscape`** — tiered competitor map with per-tier threat
   cases, pressure-tested TAM/SAM/SOM, buyer landscape, and three 2-year trends
   judged tailwind/headwind for this hypothesis.
3. **`sandbox-competitor-complaint-mining`** — what competitors' own customers say
...
4. **`sandbox-interview-kit-design`** — target profile, reachability map, and
   per-persona question sets audited against leading/socially-desirable framing.
   Takes the stress-tested hypothesis as input.
5. **`sandbox-outreach-pipeline`** *(hybrid)* — prospect list with verified-vs-
...
6. **`sandbox-interview-synthesis`** — after interviews accumulate: per-interview
...
7. **`sandbox-solution-concept-pressure-test`** — once validation holds, develop
...
8. **`sandbox-prototype-sprint`** — build only the single core interaction, plus a
...
1. **`sandbox-mvp-architecture-charter`** — before production code: the
...
2. **`sandbox-mvp-scope-guardrail`** (CHARTER mode) — what the MVP does, what it
...
3. *Build with your coding agent* — `sandbox-feature-work` (already in this repo)
   drives substantial changes, governed by the charter and scope doc.
4. **`sandbox-metrics-framework`** — metrics that matter, activation and D7/D30
   benchmarks, and this product's false-positive signatures — defined before the
   first user shows up.
5. **`sandbox-prelaunch-security-review`** — auth/session, data exposure, input
...
6. **`sandbox-feedback-loop-ops`** *(hybrid, recurring)* — weekly: intake and
...
7. **`sandbox-pmf-diagnostic`** — Sean Ellis test (>40% "very disappointed" among
...
8. **`sandbox-pivot-diagnostic`** — only after 3+ iteration cycles without
...
1. **`sandbox-tech-debt-audit`** — the MVP codebase's debt, sequenced across
   ...
   `sandbox-quality-improvement` / `sandbox-feature-work`.
2. **`sandbox-compliance-review`** — code-level findings mapped to the target
...
3. **`sandbox-founder-load-audit`** — every recurring task, decision, and
   ...
   candidates.
4. **`sandbox-product-ops-system`** *(hybrid)* — sprint cadence, minimum spec
...
1. **`sandbox-bottleneck-stress-test`** — the founder-bottleneck map plus a
   ...
   failure point.
2. **`sandbox-enterprise-procurement-pack`** *(hybrid)* — the written pack
...
3. **`sandbox-enterprise-gap-analysis`** — for three named target accounts (or
...
4. **`sandbox-gtm-engine`** — segmentation, messaging architecture, and playbooks
...
5. **`sandbox-domain-knowledge-codify`** — the proprietary knowledge substrate:
   a searchable context library plus codified workflow routines, each
   replay-verified to run the same way every time.
6. **`sandbox-moat-test-suite`** — vertical edge cases a generic competitor gets
   wrong, landed as an accreting scenario-test suite whose coverage map *is* the
   moat map.
7. **`sandbox-data-flywheel-audit`** — what interaction data exists (honest
...
8. **`sandbox-lockin-audit`** — per-customer integration and switching-cost
...
**`sandbox-playbook-skill-forge`** is the pipeline that produced this family:
